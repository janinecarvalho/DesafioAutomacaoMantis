...
namespace DesafioAutomacaoMantis.Helpers
{
    public class ExtentReportHelpers
    {
        ...
        static string reportName = JsonBuilder.GetAppSettings("reportName") + "_" + DateTime.Now.ToString("dd-MM-yyyy_HH-mm");
        ...
        public static void CreateStep()
        {
            ...
            {
                if (JsonBuilder.GetAppSettings("GET_SCREENSHOT_FOR_EACH_STEP").Equals("true"))
                {
                    switch (stepType)
                    {
                        case "Given":
